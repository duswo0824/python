...
while True: # 무한으로 돌아가게 하기 위해 True
    ...
    #에러없이 종료할 수 있는 방법
    if (idx == len(b)-1) & (b[len(b)-1] != 3): # idx가 마지막 방 까지 왔고 그 값이 3이 아닐때
        break # 멈춤
    
    if (idx == len(b)-1) & (b[len(b)-1] == 3): # idx가 마지막 방 까지 왔고 그 값이 3이 아닐때
        break

    '''if idx == len(b)-1: # idx가 마지막 방까지 왔을 때 멈춤(마지막 인덱스 확인불가ㅜ)
        break'''


    # len(b)-1 이 마지막 index(방) 10번째 방이 있다는 뜻
    # b[len(b)-1] : b 리스트의 마지막 값 10번째 방에 있는게 2라는 뜻
